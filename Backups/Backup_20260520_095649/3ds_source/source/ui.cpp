...
void V2UI::init() {
    ...
    hudPrevBtn      = { 3, 210, 24, 26, "[<]", CLR_BTN, false };
    hudNextBtn      = { 29, 210, 24, 26, "[>]", CLR_BTN, false };
    hudShowMapBtn   = { 57, 210, 102, 26, "SHOW MAP", CLR_BTN, false };
    hudShowGuideBtn = { 163, 210, 102, 26, "SHOW GUIDE", CLR_BTN, false };

    // Initialize HUD Back/Exit Button (touch-only exit from emulation)
    hudBackBtn      = { 293, 210, 24, 26, "HUD_BACK", C2D_Color32(80, 20, 20, 255), false };
...
void V2UI::drawButton(Button& btn) {
    ...
    if (btn.pressed) {
        ...
        color = C2D_Color32(120, 30, 30, 220);
    ...
    // Draw curved return arrow icon for HUD back button (no text, icon only)
    if (strcmp(btn.label, "HUD_BACK") == 0) {
        float cx = btn.x + btn.w / 2.0f;
        float cy = btn.y + btn.h / 2.0f + 1.0f;
        float r = 5.5f;
        u32 ic = activeTextColor;
        float lw = 1.5f;

        // Draw semicircle arc from right to left (through the top)
        int segs = 8;
        for (int i = 0; i < segs; i++) {
            float a1 = (float)i / (float)segs * M_PI;
            float a2 = (float)(i + 1) / (float)segs * M_PI;
            C2D_DrawLine(
                cx + r * cosf(a1), cy - r * sinf(a1), ic,
                cx + r * cosf(a2), cy - r * sinf(a2), ic,
                lw, 0.5f);
        }

        // Short tail dropping down from the right end of the arc
        C2D_DrawLine(cx + r, cy, ic, cx + r, cy + 4.0f, ic, lw, 0.5f);

        // Arrowhead at the left end pointing left
        float ax = cx - r, ay = cy;
        C2D_DrawLine(ax, ay, ic, ax + 3.5f, ay - 3.0f, ic, lw, 0.5f);
        C2D_DrawLine(ax, ay, ic, ax + 3.5f, ay + 3.0f, ic, lw, 0.5f);
    }
}
