...
void V2UI::init() {
    ...
    // Left: nav arrows | Middle: battery + time | Right: M / G
    hudPrevBtn      = { 4,   216, 24, 20, "[<]", CLR_BTN, false };
    hudNextBtn      = { 30,  216, 24, 20, "[>]", CLR_BTN, false };
    hudShowMapBtn   = { 268, 216, 24, 20, "M",   CLR_BTN, false };
    hudShowGuideBtn = { 294, 216, 24, 20, "G",   CLR_BTN, false };
    ...
    C2D_TextParse(&hudBackText, staticTextBuf, hudBackBtn.label);
    C2D_TextOptimize(&hudBackText);
    ...
    C2D_TextParse(&versionText, staticTextBuf, "V2NES v1.8.3");
...
void V2UI::drawButton(Button& btn) {
    ...
    else if (strcmp(btn.label, "SHOW MAP") == 0 || strcmp(btn.label, "[M]") == 0 || strcmp(btn.label, "M") == 0) text = &hudShowMapText;
    else if (strcmp(btn.label, "SHOW GUIDE") == 0 || strcmp(btn.label, "[G]") == 0 || strcmp(btn.label, "G") == 0) text = &hudShowGuideText;
...
void V2UI::renderBottom(GameState state, V2Settings& settings) {
    ...
    if (state == STATE_STARTUP) {
        // Startup splash — bottom screen is just the theme background, nothing drawn.
        ...
        // Bottom screen shows nothing during loading — top screen has the logo + status.
        ...
        if (!nesGameplayActive) {
            ...
            content.drawMap(214.0f);
        } else {
            content.drawWalk(scrollOffset);
        }

        // Draw sleek glass background overlay bar for HUD (thinner: 26px)
        C2D_DrawRectSolid(0, 214, 0.5f, 320, 26, activeHudBg);
        ...
        // Middle: battery % (right-of-center) + time (left-of-center)
        {
            ...
            const float midX   = 160.0f; // center of the 320px bar
            const float gap    = 6.0f;
            const float textY  = 220.0f;
            ...
            // Time
            static C2D_TextBuf hudClockBuf = C2D_TextBufNew(32);
            C2D_TextBufClear(hudClockBuf);
            time_t rawtime; struct tm* ti;
            time(&rawtime); ti = localtime(&rawtime);
            char timeBuf[10];
            snprintf(timeBuf, sizeof(timeBuf), "%02d:%02d", ti->tm_hour, ti->tm_min);
            C2D_Text timeTxt;
            C2D_TextParse(&timeTxt, hudClockBuf, timeBuf);
            C2D_TextOptimize(&timeTxt);
            C2D_DrawText(&timeTxt, C2D_WithColor,
                midX + gap, textY,
                0.5f, scale, scale, activeTextColor);
        }
    }
