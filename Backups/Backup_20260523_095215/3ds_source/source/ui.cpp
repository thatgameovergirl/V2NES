...
void V2UI::init() {
    ...
    // Compact bar: thin buttons, battery instead of [X], 26px tall starting at y=214
    hudPrevBtn      = { 6, 216, 24, 20, "[<]", CLR_BTN, false };
    hudNextBtn      = { 34, 216, 24, 20, "[>]", CLR_BTN, false };
    hudShowMapBtn   = { 62, 216, 40, 20, "[M]", CLR_BTN, false };
    hudShowGuideBtn = { 106, 216, 40, 20, "[G]", CLR_BTN, false };
...
void V2UI::drawButton(Button& btn) {
    ...
    else if (strcmp(btn.label, "SHOW MAP") == 0 || strcmp(btn.label, "MAPS") == 0 || strcmp(btn.label, "[M]") == 0) text = &hudShowMapText;
    else if (strcmp(btn.label, "SHOW GUIDE") == 0 || strcmp(btn.label, "GUIDE") == 0 || strcmp(btn.label, "[G]") == 0) text = &hudShowGuideText;
...
void V2UI::renderBottom(GameState state, V2Settings& settings) {
    ...
    if (state == STATE_STARTUP) {
        ...
        // Battery on the right side of the HUD bar
        {
            float batScale = 0.45f;
            float batW, batH;
            C2D_TextGetDimensions(&batteryText, batScale, batScale, &batW, &batH);
            C2D_DrawText(&batteryText, C2D_WithColor,
                SCREEN_WIDTH - batW - 8.0f, 218.0f,
                0.5f, batScale, batScale, batteryColor);
        }
    }
