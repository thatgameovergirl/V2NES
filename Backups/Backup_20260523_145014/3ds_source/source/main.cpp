...
static void initLogoTexture() {
    ...
    // Crop to the 469×255 content region centred in the 512×256 atlas
    // (21px black padding on left, 0px on top — from the build-time resize).
    // This fills the full 320px content area instead of showing 292px of logo
    // surrounded by black margins.
    g_logoSubTex.width  = 469;
    g_logoSubTex.height = 255;
    g_logoSubTex.left   = 21.0f  / 512.0f;
    g_logoSubTex.top    = 1.0f;
    g_logoSubTex.right  = 490.0f / 512.0f;
    g_logoSubTex.bottom = 1.0f   / 256.0f;
...
int main(int argc, char* argv[])
{
	...
	C2D_TextBuf dbgOverlayBuf = C2D_TextBufNew(256);
	...
	while (aptMainLoop())
	{
		...
		if (kHeld & KEY_TOUCH) {
			...
			ui.themeCatBtn.pressed = ui.speedCatBtn.pressed = false;
		...
		if (touchPending && (kHeld & KEY_TOUCH)) {
			...
			if (touch.px != 0 || touch.py != 0) {
				...
				if (ui.checkTouch(touch, ui.themeCatBtn)) {
					// Cycle to next accent colour — no sub-screen, instant feedback.
					int acc = ((int)settings.getAccent() + 1) % ACCENT_COUNT;
					settings.setAccent((AccentColor)acc);
					settings.save();
					snprintf(topNotifyMsg, sizeof(topNotifyMsg), "Theme: %s", settings.getAccentName((AccentColor)acc));
					topNotifyTimer = 120;
				}
				...
				if (ui.checkTouch(touch, ui.hudShowMapBtn)) {
					...
					if (ui.gameplayMode == GAMEPLAY_GUIDE) {
						...
						state = STATE_WALK_LIST;
						cameFromPlaying = true;
						ui.selectedIndex = (ui.activeWalkIndex >= 0) ? ui.activeWalkIndex : 0;
					} else {
						ui.gameplayMode = GAMEPLAY_GUIDE;
		...
		if (state == STATE_MAP_LIST || state == STATE_WALK_LIST || state == STATE_GAME_LIST) {
			int rawSize = 0;
			if (state == STATE_MAP_LIST)       rawSize = (int)ui.content.mapFiles.size();
			else if (state == STATE_WALK_LIST)  rawSize = (int)ui.content.walkFiles.size();
			else                                rawSize = (int)ui.content.romFiles.size();
			int listSize = (rawSize < 5) ? rawSize : 5;

			if (listSize > 0) {
				if (kDown & KEY_DOWN)
					ui.selectedIndex = (ui.selectedIndex + 1) % listSize;
				if (kDown & KEY_UP)
					ui.selectedIndex = (ui.selectedIndex - 1 + listSize) % listSize;
			}

			if ((kDown & KEY_A) && ui.selectedIndex >= 0 && ui.selectedIndex < listSize) {
				if (state == STATE_MAP_LIST) {
					if (ui.content.loadMap(ui.content.mapFiles[ui.selectedIndex].path.c_str())) {
						currentMapIndex = ui.selectedIndex;
						ui.activeMapIndex = ui.selectedIndex;
					...
					if (ui.content.loadWalk(ui.content.walkFiles[ui.selectedIndex].path.c_str())) {
						ui.activeWalkIndex = ui.selectedIndex;
						ui.scrollOffset = ui.content.loadWalkPosition();
		...
		if (state == STATE_EMULATING) {
			...
			if (framesDue > 4) framesDue = 4;   // cap to avoid death spiral
			lastEmuTime += (u64)framesDue * 16;

			for (int f = 0; f < framesDue; f++) {
				nesEmulateFrame();
				// Submit audio after each frame so no samples are lost
				if (audioOk) {
		...
		if (C3D_FrameBegin(C3D_FRAME_SYNCDRAW)) {
			...
			if (topTarget) {
				...
				if (showLogo && g_logoReady) {
					// Scale logo to fill almost the full top-screen height (~212px on 240px panel).
					// Width at this scale is ~390px, centred with 5px side margins.
					float scale = 390.0f / 469.0f;
					C2D_DrawImageAt(g_logoImage, 5, 14, 0.7f, NULL, scale, scale);
				}
				...
				if (nesRomLoaded && nesImage.tex && state != STATE_LOADING_GAME) {
					// Upload the current frame's pixels before drawing
				...
				// Debug overlay (top-right corner): NES registers + live free RAM.
				// Own buffer (dbgOverlayBuf) — never cleared by the notification toast.
				// Content refreshed ~4 Hz (every 15 frames) to avoid per-frame parse cost.
				{
					static C2D_Text dbgRegTxt = {}, dbgRamTxt = {};
					static u32  dbgFrames = 0;   // counts down; refresh when it hits 0
					static bool dbgReady  = false;

					if (dbgFrames == 0) {
						u32 freeKB = (u32)(osGetMemRegionFree(MEMREGION_APPLICATION) / 1024);
						C2D_TextBufClear(dbgOverlayBuf);
						if (nesRomLoaded) {
							char regBuf[64];
							...
							C2D_TextParse(&dbgRegTxt, dbgOverlayBuf, regBuf);
							C2D_TextOptimize(&dbgRegTxt);
						}
						char ramBuf[32];
						snprintf(ramBuf, sizeof(ramBuf), "Free:%5lu KB", (unsigned long)freeKB);
						C2D_TextParse(&dbgRamTxt, dbgOverlayBuf, ramBuf);
						C2D_TextOptimize(&dbgRamTxt);
						dbgReady  = true;
						dbgFrames = 15;   // ~4 Hz at 60 fps
					} else {
						dbgFrames--;
					}
					...
					if (dbgReady) {
						if (nesRomLoaded) {
							C2D_DrawRectSolid(224, 2, 0.85f, 174, 44, C2D_Color32(0, 0, 0, 180));
							C2D_DrawText(&dbgRegTxt, C2D_WithColor, 228,  4, 0.9f, 0.4f, 0.4f, C2D_Color32(255, 255,   0, 255));
							C2D_DrawText(&dbgRamTxt, C2D_WithColor, 228, 32, 0.9f, 0.4f, 0.4f, C2D_Color32(  0, 255, 150, 255));
						} else {
							C2D_DrawRectSolid(224, 2, 0.85f, 174, 18, C2D_Color32(0, 0, 0, 180));
							C2D_DrawText(&dbgRamTxt, C2D_WithColor, 228,  4, 0.9f, 0.4f, 0.4f, C2D_Color32(  0, 255, 150, 255));
						}
					}
				}
