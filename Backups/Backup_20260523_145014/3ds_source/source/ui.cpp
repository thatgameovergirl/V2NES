...
void V2UI::init() {
    ...
    // Initialize Settings Category Buttons (Layout removed)
    themeCatBtn  = { 40, 45, 240, 40, "[ THEME ]", CLR_BTN, false };
    speedCatBtn  = { 40, 100, 240, 40, "[ SENSITIVITY ]", CLR_BTN, false };
    ...
    hudNextBtn      = { 60,  211, 60, 27, "FOWD",   CLR_BTN, false };
    ...
    C2D_TextParse(&hudNextText, staticTextBuf, "FOWD");
    ...
    C2D_TextParse(&versionText, staticTextBuf, "V2NES v1.8.4");
...
void V2UI::drawButton(Button& btn) {
    ...
    else if (strcmp(btn.label, "FOWD") == 0) text = &hudNextText;
    ...
    else if (strncmp(btn.label, "Theme:", 6) == 0) {
        C2D_TextParse(&themeCatText, dynamicTextBuf, btn.label);
    ...
    if (text) {
        ...
        if (strncmp(btn.label, "ZOOM:", 5) == 0 || strncmp(btn.label, "PAN:", 4) == 0 ||
            strncmp(btn.label, "SCROLL:", 7) == 0 || strncmp(btn.label, "Theme:", 6) == 0) {
...
void V2UI::renderBottom(GameState state, V2Settings& settings) {
    ...
    // Theme button is tinted with the active accent so the current colour is visible at a glance.
    themeCatBtn.color = activeAccentColor;
    ...
    if (state == STATE_STARTUP) {
        ...
        // Theme button label is dynamic — update it to show the current accent name.
        static char themeLabel[40];
        snprintf(themeLabel, sizeof(themeLabel), "Theme: %s", settings.getAccentName(settings.getAccent()));
        themeCatBtn.label = themeLabel;
