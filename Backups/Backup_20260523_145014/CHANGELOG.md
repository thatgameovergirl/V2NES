...
* **HUD Buttons Renamed and Resized for Finger Use**:
  * `<` → **BACK**, `>` → **FWD**, `M` → **MAPS**, `G` → **GUIDES**.
  * All four buttons now fill the full HUD bar height (27 px tall) and are significantly wider: BACK 54 px, FWD 46 px, MAPS 52 px, GUIDES 60 px. Previously all four were 24×25 px icon-only tiles that were too small to reliably tap.
...
* **Background Emulation Thread (Core 1)**:
  * `nesEmulateFrame()` has been moved off the main (render) thread onto a dedicated background thread pinned to CPU core 1.
  * The emu is kicked at the top of each loop iteration via `nesKickFrame()` and runs in parallel with input processing and `C3D_FrameBegin`'s GPU-sync block — the heaviest blocking point in the loop.
  * `nesWaitFrame()` collects the result just before the NES texture is read; by that point the GPU sync has usually consumed the full emulation budget so the wait is near-zero.
  * Audio is submitted immediately after `nesWaitFrame()` while the APU is idle, eliminating the previous per-frame audio/emu interleaving.
  * Graceful sync fallback: if thread creation fails, `nesKickFrame()`/`nesWaitFrame()` automatically run synchronously.
  * `nesStopThread()` joins the thread cleanly before hardware teardown on app exit.
