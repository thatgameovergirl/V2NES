...
bool V2Content::loadMap(const char* path) {
    ...
    if (memcmp(magic, "VM\x02\0", 4) == 0) {
        ...
        printf("\n[MAP] Tiled: grid=%ux%u size=%ux%u aspect=%.3f", gridCols, gridRows, originalWidth, originalHeight, mapAspect);

        u32 tileSize = 1024 * 1024 * 2; // RGB565 (2 bytes/pixel)
        u8* tempLinear = (u8*)malloc(tileSize);
        u8* gpuTiled = (u8*)linearAlloc(tileSize);

        if (!tempLinear || !gpuTiled) {
        ...
        for (u32 r = 0; r < gridRows; r++) {
            for (u32 c = 0; c < gridCols; c++) {
                ...
                if (read != tileSize) {
                    printf("\n[MAP] ERROR: Read %zu/%u bytes on tile %u,%u", read, tileSize, r, c);
                    free(tempLinear);
                    linearFree(gpuTiled);
                    fclose(f);
                    return false;
                ...
                // Swizzle RGB565 data for 3DS PICA200 GPU
                ...
                C3D_TexInit(&tile->tex, 1024, 1024, GPU_RGB565);
        ...
        fclose(f);
        mapLoaded = true;
        printf("\n[MAP] SUCCESS: Loaded %u tiles!", (unsigned int)mapTiles.size());
        return true;
    } else {
        ...
        tile->subTex.bottom = 1.0f - (float)tH / 1024.0f;
...
void V2Content::drawMap(float viewportH) { 
    if (mapLoaded) {
        printf("\x1b[16;1H[RENDER] zoom=%.2f panX=%.1f panY=%.1f", zoom, panX, panY);
...
void V2Content::resetView(float viewportH) {
    zoom = 1.0f;
    panX = 0.0f;
    float mapHeight = 320.0f / mapAspect;
...
void V2Content::saveWalkPosition(int currentScroll) { 
    ...
    std::string posPath = currentWalkPath; 
