...
int main(int argc, char* argv[])
{
	...
	// Initialize console on top screen for debug/status
	PrintConsole topConsole;
	consoleInit(GFX_TOP, &topConsole);
	consoleSelect(&topConsole);

	printf("\x1b[1;1HV2NES 3DS - Control Center");
	printf("\x1b[3;1HLayout: %s", settings.getLayoutName(settings.getLayout()));
	printf("\x1b[5;1HL: Quick Save | R: Quick Load");
			...
			if (touch.px != 0 || touch.py != 0) {
				...
				if (ui.checkTouch(touch, ui.playBtn)) {
					...
					if (!ui.content.mapFiles.empty()) {
						...
						ui.content.freeMap();
						ui.activeMapIndex = -1;
						ui.showNotification("No Maps Found! Copy maps to SD card.");
					}
